    ...
    if (e.type == "keydown" && e.which == 119 || e.type == 'click') {
        _ventanaDatos({
            ...
            callback: function (data) {
                ...
                $('#descripGrcp71C').val(data.nombre);
...
function _eliminaDatos71C() {
    ...
    LLAMADO_DLL({
        dato: [$_NovedSer71C, $_COD71C],
        callback: function (data) { validarResp_71C(data, $_COD71C) },
        nombredll: 'SAL71C-02',
        carpeta: 'SALUD'
    })
}
...
function envioDatSer71C() {
    ...
    LLAMADO_DLL({
        dato: [novd71C, $_COD71C, desc71C, procedm71C, laborat71C, imagen71C, servicios71C, consulta71C, patologia71C, pyp71C, medicamn71C],
        callback: function (data) {
            validarResp_71C(data, $_COD71C, desc71C)
        },
        nombredll: 'SAL71C-02',
        carpeta: 'SALUD'
    })
...
function validarResp_71C(data, $_COD71C, desc71C) {
    loader('hide');
    var rdll = data.split('|');
    console.log(rdll[0])
    if (rdll[0].trim() == '00') {
        switch (parseInt($_NovedSer71C)) {
            case 7:
                _consultaSql({
                    sql: `INSERT INTO sc_grupocap VALUES ('${$_COD71C}', '${desc71C}');`,
                    db: $CONTROL,
            ...
            case 9:
                _consultaSql({
                    sql: `DELETE FROM sc_grupocap WHERE codigo = '${$_COD71C}'`,
                    db: $CONTROL,
                    callback: function (error, results, fields) {
                        if (error) throw error;
                        else {
